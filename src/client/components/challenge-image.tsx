...
import devicon from "../assets/devicon.json";
...
export interface IChallengeImageProps extends React.PropsWithoutRef<any> {
  skills: string[];
}

function fillTechLogos(skills: string[]): IDevicon[] {
  const techs: IDevicon[] = Array.from(devicon);
  const used = techs.filter((v) => skills.includes(v.name));
  return used.map((v) => ({
    ...v,
...
export function ChallengeImage(props: IChallengeImageProps) {
  const [loaded, setLoaded] = React.useState(false);
  const technologies = fillTechLogos(props.skills);
  return (
    <div
      data-component="challenge-image"
      className={`flex overflow-hidden h-full opacity-0 ${
        loaded ? "challenge-image-transition" : ""
      }`}>
      {technologies.map((tech, index) => (
        <div className="flex-grow relative h-full" key={index}>
            ...
            <div className="w-full h-full p-3 flex items-center justify-center">
              <div
                ...
                <svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%">
                  <defs>
                    <pattern id="circuit" width="304" height="304" patternUnits="userSpaceOnUse">
                      <path
                        fill="black"
